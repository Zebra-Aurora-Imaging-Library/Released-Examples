...
namespace MCol
{
    class Program
    {
        ...
        //*****************************************************************************/
        // Source image
        const string RGB_IMAGE_FILE = MIL.M_IMAGE_PATH + "BinderClip.mim";

        static void RGBtoGrayscaleExample(MIL_ID MilSystem, MIL_ID MilDisplay)
            {
            ...
            MIL_ID DisplayImage     = MIL.M_NULL,     // Display image buffer identifier.
                   SourceChild      = MIL.M_NULL,     // Source image child buffer identifier.
                   LuminanceChild   = MIL.M_NULL,     // Luminance image child buffer identifier.
                   PCPchild         = MIL.M_NULL,     // PCP image child buffer identifier.      
                   LuminanceResult  = MIL.M_NULL,     // Luminance result identifier.
                   PCPresult        = MIL.M_NULL,     // PCP result identifier.            
                   Mask             = MIL.M_NULL,     // Mask identifier to perform PCP. 
                   ChildMask        = MIL.M_NULL;     // Child mask identifier to perform PCP. 

            // Inquire size and type of the source image.
            MIL_INT SourceSizeX = MIL.MbufDiskInquire(RGB_IMAGE_FILE, MIL.M_SIZE_X, MIL.M_NULL);
            MIL_INT SourceSizeY = MIL.MbufDiskInquire(RGB_IMAGE_FILE, MIL.M_SIZE_Y, MIL.M_NULL);
            MIL_INT Type        = MIL.MbufDiskInquire(RGB_IMAGE_FILE, MIL.M_TYPE, MIL.M_NULL);

            // Allocate buffer to display the input image and the results.
            MIL.MbufAllocColor(MilSystem, 3, SourceSizeX,
                               3 * SourceSizeY + 2 * DISPLAY_CENTER_MARGIN_Y, Type,
                               MIL.M_IMAGE + MIL.M_PROC + MIL.M_DISP, ref DisplayImage);
            MIL.MbufClear(DisplayImage, 0);

            // Create a source child in the display image.
            MIL.MbufChildColor2d(DisplayImage, MIL.M_ALL_BANDS, 0, 0, SourceSizeX, SourceSizeY,
                                 ref SourceChild);

            // Create a destination child in the display image for luminance based conversion.
            MIL.MbufChildColor2d(DisplayImage, MIL.M_ALL_BANDS, 0,
            ...
            MIL.MbufAlloc2d(MilSystem, SourceSizeX, SourceSizeY, Type, MIL.M_IMAGE + MIL.M_PROC, ref LuminanceResult);
            ...
            Console.Write("Press <Enter> to convert the image using principal ");
            Console.Write("component projection\ntechnique. \n\n");
            Console.ReadKey();

            // Create a mask to identify important colors in the source image.
            MIL.MbufAlloc2d(MIL.M_DEFAULT_HOST, SourceSizeX, SourceSizeY, 8 + MIL.M_UNSIGNED, MIL.M_IMAGE + MIL.M_PROC,
                            ref Mask);
            ...
            MIL_INT OffsetX   = 105,
                    ...
                    MaskSizeX = 60,
                    MaskSizeY = 20;
            MIL.MbufChild2d(Mask, OffsetX, OffsetY, MaskSizeX, MaskSizeY, ref ChildMask);
            ...
            MIL.MbufAlloc2d(MilSystem, SourceSizeX, SourceSizeY, Type, MIL.M_IMAGE + MIL.M_PROC, ref PCPresult);
            MIL.McolProject(SourceChild, Mask, PCPresult, MIL.M_NULL,
                            MIL.M_PRINCIPAL_COMPONENT_PROJECTION, MIL.M_DEFAULT, MIL.M_NULL);
