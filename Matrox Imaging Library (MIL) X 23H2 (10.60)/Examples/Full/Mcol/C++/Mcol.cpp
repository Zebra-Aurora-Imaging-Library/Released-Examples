...
void RGBtoGrayscaleExample(MIL_ID MilSystem, MIL_ID MilDisplay);
...
*****************************************************************************/
/* Source image */
#define RGB_IMAGE_FILE  M_IMAGE_PATH MIL_TEXT("BinderClip.mim")

void RGBtoGrayscaleExample(MIL_ID MilSystem, MIL_ID MilDisplay)
   {
   MosPrintf(MIL_TEXT("\nCONVERSION FROM RGB TO GRAYSCALE:\n"));
   MosPrintf(MIL_TEXT("---------------------------------\n"));
   MosPrintf(MIL_TEXT("The example compares principal component projection to "));
   MosPrintf(MIL_TEXT("luminance based\nconversion.\n\n"));
   ...
   MIL_INT SourceSizeX = MbufDiskInquire(RGB_IMAGE_FILE, M_SIZE_X, M_NULL);
   MIL_INT SourceSizeY = MbufDiskInquire(RGB_IMAGE_FILE, M_SIZE_Y, M_NULL);
   MIL_INT Type        = MbufDiskInquire(RGB_IMAGE_FILE, M_TYPE, M_NULL);

   /* Allocate buffer to display the input image and the results. */
   MbufAllocColor(MilSystem, 3, SourceSizeX, 3 * SourceSizeY + 2 * DISPLAY_CENTER_MARGIN_Y,
   ...
   MbufAlloc2d(MilSystem, SourceSizeX, SourceSizeY, Type, M_IMAGE + M_PROC, &LuminanceResult);
   MimConvert(SourceChild, LuminanceResult, M_RGB_TO_L);
   MbufCopy(LuminanceResult, LuminanceChild);

   MosPrintf(MIL_TEXT("Color image converted to grayscale using luminance based ")
             MIL_TEXT("conversion\ntechnique.\n\n"));
   MosPrintf(MIL_TEXT("Press <Enter> to convert the image using principal ")
             MIL_TEXT("component projection\ntechnique.\n\n"));
   MosGetch();
   ...
   MbufAlloc2d(MilSystem, SourceSizeX, SourceSizeY, Type, M_IMAGE + M_PROC, &PCPresult);
   ...
   MosPrintf(MIL_TEXT("Color image converted to grayscale using principal ")
             MIL_TEXT("component projection\ntechnique.\n"));
   MosPrintf(MIL_TEXT("Press <Enter> to end.\n\n"));
   MosGetch();
