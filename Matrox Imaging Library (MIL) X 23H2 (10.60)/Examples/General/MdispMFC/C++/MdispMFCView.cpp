...
const MIL_INT CMdispMFCView::s_Opacity[] =
                                 {M_DEFAULT, M_DISABLE, 0, 20, 40, 60, 80, 100};
...
const UINT CMdispMFCView::s_FirstOverlayOpacityControlInRange = ID_OVERLAY_OPACITY_DEFAULT;
const UINT CMdispMFCView::s_FirstGraOpacityControlInRange = ID_GRA_OPACITY_DEFAULT;
...
BEGIN_MESSAGE_MAP(CMdispMFCView, CView)
   ...
   ON_WM_DESTROY()
   ON_COMMAND(ID_OVERLAY_SHOW, OnOverlay)
   ON_UPDATE_COMMAND_UI(ID_OVERLAY_SHOW, OnUpdateOverlay)
   ON_WM_SIZE()
   ON_WM_TIMER()
   ...
   ON_COMMAND(ID_GRA_DRAW_DIRECTION, &CMdispMFCView::OnGraDrawDirCycle)
   ON_UPDATE_COMMAND_UI(ID_GRA_DRAW_DIRECTION, &CMdispMFCView::OnUpdateGraDrawDirCycle)
   ON_COMMAND(ID_GRA_ORIENTED_RECT, &CMdispMFCView::OnGraDrawOrientedRect)
   ON_UPDATE_COMMAND_UI(ID_GRA_ORIENTED_RECT, &CMdispMFCView::OnUpdateGraDrawOrientedRect)
   ON_COMMAND(ID_GRA_ARC_THREE_POINTS, &CMdispMFCView::OnGraDrawThreePointsArc)
   ON_UPDATE_COMMAND_UI(ID_GRA_ARC_THREE_POINTS, &CMdispMFCView::OnUpdateGraDrawThreePointsArc)
   ON_COMMAND_RANGE(ID_GRA_OPACITY_DEFAULT, ID_GRA_OPACITY_100, &CMdispMFCView::OnGraOpacity)
   ON_UPDATE_COMMAND_UI_RANGE(ID_GRA_OPACITY_DEFAULT, ID_GRA_OPACITY_100, &CMdispMFCView::OnUpdateGraOpacity)
   ON_COMMAND_RANGE(ID_OVERLAY_OPACITY_DEFAULT, ID_OVERLAY_OPACITY_100, &CMdispMFCView::OnOverlayOpacity)
   ON_UPDATE_COMMAND_UI_RANGE(ID_OVERLAY_OPACITY_DEFAULT, ID_OVERLAY_OPACITY_100, &CMdispMFCView::OnUpdateOverlayOpacity)
   ON_COMMAND(ID_GRA_LINE_THICKNESS, &CMdispMFCView::OnGraLineThicknessToggle)
   ON_UPDATE_COMMAND_UI(ID_GRA_LINE_THICKNESS, &CMdispMFCView::OnUpdateGraLineThicknessToggle)
  //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CMdispMFCView::OnDraw(CDC* pDC) 
   ...
   // If no windowed display involved, no window annotations can be written in display.
      ...
      if(m_isGDIAnnotationsEnabled )
         ...
         clientRECT.top += 50; // move down to avoid collision with Interactive Graphic Annotations
...
void CMdispMFCView::OnGDIAnnotations() 
   {
   /////////////////////////////////////////////////////////////////////////
   // MIL: Write code that will be executed when 'GDI Annotations' button or menu is clicked
   /////////////////////////////////////////////////////////////////////////   
   ...
   if(!m_isGDIAnnotationsEnabled)
      ...
      //Make sure it was supported before setting it's status to enabled [CALL TO MIL]
   ...
   //Generate an update to make sure an OnDraw message is sent
   InvalidateRect(NULL);
   UpdateWindow();
   }
   ...
   if(m_MilDisplay)
      ...
      if(m_bGraphicsAnnotations)
         ...
         if(!m_MilGraphContext && !m_MilGraphList)
            ...
            MgraControl(m_MilGraphContext, M_FONT_SIZE, 12);// Because of Large Font reduce size from 24 to 12 
            MgraFont(m_MilGraphContext, M_FONT_DEFAULT_TTF MIL_TEXT(":Bold"));
            ...
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_ACTION_KEYS, M_ENABLE);
            
            //Remap rotation to page down/up keys to prevent problems with alt key and mfc while using action keys
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_ACTION_KEY_ROTATE_COUNTER_CLOCKWISE, M_KEY_PAGEUP);
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_ACTION_KEY_ROTATE_CLOCKWISE, M_KEY_PAGEDOWN);
            
            //Remap angle snapping on rotation to shift to prevent problems with alt key and mfc while using modifier keys
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_MODE_ROTATE_ALT, M_DISABLE);
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_MODE_ROTATE_SHIFT, M_ANGLE_SNAPPING);
            
            //Disable resize with aspect ratio set to 1 to prevent problems with alt key and mfc while using modifier keys
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_MODE_RESIZE_ALT, M_DISABLE);
            
            //Map escape and delete keys to control the graphic list
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_ACTION_KEY_CANCEL, M_KEY_ESC);
            MgraControlList(m_MilGraphList, M_LIST, M_DEFAULT, M_ACTION_KEY_DELETE, M_KEY_DELETE);
...
void CMdispMFCView::OnGraOpacity(UINT nID)
   {
   /////////////////////////////////////////////////////////////////////////
   // MIL: Write code that will be executed when 'Graphics Annotations->Opacity' menu item is clicked
   /////////////////////////////////////////////////////////////////////////
   UINT OpacityIndex = nID - s_FirstGraOpacityControlInRange;
   ASSERT(OpacityIndex < (sizeof(s_Opacity) / sizeof(MIL_INT)));

   if(m_bGraphicsAnnotations && m_MilDisplay)
      {
      // Apply Opacity to display [CALL TO MIL]
      MdispControl(m_MilDisplay, M_GRAPHIC_LIST_OPACITY, s_Opacity[OpacityIndex]);
      
      // Check if control worked correctly before considering it successful [CALL TO MIL]
      if((s_Opacity[OpacityIndex] == M_DEFAULT) || (MdispInquire(m_MilDisplay, M_GRAPHIC_LIST_OPACITY, M_NULL) == s_Opacity[OpacityIndex]))
         {
         m_currentGraOpacity = s_Opacity[OpacityIndex];
         }
      }
   }
void CMdispMFCView::OnUpdateGraOpacity(CCmdUI *pCmdUI)
   {
   pCmdUI->Enable(m_bGraphicsAnnotations);
   UINT OpacityIndex = pCmdUI->m_nID - s_FirstGraOpacityControlInRange;
   ASSERT(OpacityIndex < (sizeof(s_Opacity) / sizeof(MIL_INT)));

   pCmdUI->SetRadio(s_Opacity[OpacityIndex] == m_currentGraOpacity);
   }

void CMdispMFCView::OnOverlayOpacity(UINT nID)
   {
   /////////////////////////////////////////////////////////////////////////
   // MIL: Write code that will be executed when 'Overlay->Opacity' menu item is clicked
   /////////////////////////////////////////////////////////////////////////
   if(m_isOverlayEnabled)
      {
      UINT OpacityIndex = nID - s_FirstOverlayOpacityControlInRange;
      ASSERT(OpacityIndex < (sizeof(s_Opacity) / sizeof(MIL_INT)));

      // Apply Opacity to display [CALL TO MIL]
      MdispControl(m_MilDisplay, M_OVERLAY_OPACITY, s_Opacity[OpacityIndex]);

      // Check if control worked correctly before considering it successful [CALL TO MIL]
      if((s_Opacity[OpacityIndex] == M_DEFAULT) || (MdispInquire(m_MilDisplay, M_OVERLAY_OPACITY, M_NULL) == s_Opacity[OpacityIndex]))
         {
         m_currentOverlayOpacity = s_Opacity[OpacityIndex];
         }
      }
   }
void CMdispMFCView::OnUpdateOverlayOpacity(CCmdUI *pCmdUI)
   {
   pCmdUI->Enable(m_isOverlayEnabled);
   UINT OpacityIndex = pCmdUI->m_nID - s_FirstOverlayOpacityControlInRange;
   ASSERT(OpacityIndex < (sizeof(s_Opacity) / sizeof(MIL_INT)));

   pCmdUI->SetRadio(s_Opacity[OpacityIndex] == m_currentOverlayOpacity);
   }
...
void CMdispMFCView::OnGraDrawOrientedRect()
   {
   if(m_bGraphicsAnnotations)
      {
...
void CMdispMFCView::OnGraDrawThreePointsArc()
   {
   if(m_bGraphicsAnnotations)
      {
      MgraColor(m_MilGraphContext, M_COLOR_GREEN);
...
void CMdispMFCView::OnGraLineThicknessToggle()
   {
   MIL_INT NbGrph = 0;
...
void CMdispMFCView::OnUpdateGraRectangle(CCmdUI *pCmdUI)
   ...
   // TODO: Add your command update UI handler code here
   pCmdUI->Enable(m_bGraphicsAnnotations);
   pCmdUI->SetCheck(m_PrimitiveInCreation == M_AXIS_ALIGNED_RECT);
   }
void CMdispMFCView::OnUpdateGraCircle(CCmdUI *pCmdUI)
   {
   // TODO: Add your command update UI handler code here
   pCmdUI->Enable(m_bGraphicsAnnotations);
   pCmdUI->SetCheck(m_PrimitiveInCreation == M_CIRCLE);
   }
...
void CMdispMFCView::OnUpdateGraDrawOrientedRect(CCmdUI *pCmdUI)
   {
   // TODO: Add your command update UI handler code here
   pCmdUI->Enable(m_bGraphicsAnnotations);
   pCmdUI->SetCheck(m_PrimitiveInCreation == M_ORIENTED_RECT);

   }
void CMdispMFCView::OnUpdateGraDrawThreePointsArc(CCmdUI *pCmdUI)
   {
   // TODO: Add your command update UI handler code here
   pCmdUI->Enable(m_bGraphicsAnnotations);
   pCmdUI->SetCheck(m_PrimitiveInCreation == M_ARC_THREE_POINTS);
   }
void CMdispMFCView::OnUpdateGraLineThicknessToggle(CCmdUI *pCmdUI)
   {
   // TODO: Add your command update UI handler code here
   pCmdUI->Enable(m_bGraphicsAnnotations);
   }
...
void CMdispMFCView::OnGraDrawDirCycle()
   {
   ...
   for(MIL_INT g = 0; g < NbGrph; g++) // Finds the highest draw direction among selected graphics
      ...
      if(MgraInquireList(m_MilGraphList, M_GRAPHIC_INDEX(g), M_DEFAULT, M_GRAPHIC_SELECTED, M_NULL) == M_TRUE)
         ...
         if(FoundIdx < NbValues)
            { DrawDirCurValueIdx = max(DrawDirCurValueIdx, FoundIdx); }
         ++NbSelectedGrph;
         }
      }
...
void CMdispMFCView::OnUpdateGraDrawDirCycle(CCmdUI *pCmdUI)
   {
   if(m_bGraphicsAnnotations)
      {
      pCmdUI->Enable(true);
