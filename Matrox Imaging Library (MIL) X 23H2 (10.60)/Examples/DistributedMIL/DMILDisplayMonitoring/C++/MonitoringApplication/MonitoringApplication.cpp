...
#define PUBLISH_SYSTEM_NAME   MIL_TEXT("dmilshmrt://localhost")
#else
// If we are connecting to the local host, use dmilshm under Windows.
// If the PublishingApplication is on another computer,
// use dmiltcp instead.
#if M_MIL_USE_LINUX
#define PUBLISH_SYSTEM_NAME   MIL_TEXT("dmiltcp://localhost")
#else
#define PUBLISH_SYSTEM_NAME   MIL_TEXT("dmilshm://localhost")
//#define PUBLISH_SYSTEM_NAME   MIL_TEXT("dmiltcp://localhost")
#endif
#endif
...
int MosMain(void)
{
   ...
   if(MilRemoteApplication == 0)
      ...
      MappInquireConnection(MilRemoteApplication, M_DMIL_PUBLISHED_NAME, 
         MIL_TEXT("DisplayImage"), M_DEFAULT, &RemoteDisplayImage);
      MappInquireConnection(MilRemoteApplication, M_DMIL_PUBLISHED_NAME, 
         MIL_TEXT("OverlayImage"), M_DEFAULT, &RemoteOverlayImage);
      ...
      if(RemoteDisplayImage)
         ...
         MdispControl(LocalDisplay, M_TITLE, WINDOW_TITLE);
