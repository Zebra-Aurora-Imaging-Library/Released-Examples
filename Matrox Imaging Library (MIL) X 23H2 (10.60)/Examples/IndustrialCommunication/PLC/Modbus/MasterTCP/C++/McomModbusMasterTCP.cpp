...
#define MODBUS_SLAVE_ADDRESS MIL_TEXT("localhost")
...
int MosMain(void)
{
   ...
   MosPrintf(MIL_TEXT("Sending triggers to ModBus slave on IP ") MODBUS_SLAVE_ADDRESS MIL_TEXT(".\nPress <Enter> to end.\n"));
...
void WriteControl(MIL_UINT8 trigger, MIL_UINT8 resultACK, MIL_UINT8 resultCopy)
{
   ...
   McomWrite(MilCom, MIL_TEXT("1@") MODBUS_SLAVE_ADDRESS MIL_TEXT("/") M_HOLDING_REGISTER, 0, sizeof(control) / sizeof(MIL_UINT16), control);
}
...
void ReadStatus(MIL_UINT8 *result)
{
   ...
   McomRead(MilCom, MIL_TEXT("1@") MODBUS_SLAVE_ADDRESS MIL_TEXT("/") M_INPUT_REGISTER, 0, sizeof(status) / sizeof(MIL_UINT16), status);
