//***************************************************************************************
// 
// File name: CylinderFinder.cpp  
//
// Synopsis: This example uses cylinder finder to define cylinder models and search for 
//           cylinders in 3D point clouds. A simple cylinder finder example is presented first
//           (multiple occurrencesin a simple scene), followed by more complex example(multiple 
//           occurrences in a complex scene with more advanced controls).
//
...
// Constants.
//*****************************************************************************
static const MIL_STRING FILENAMES[2] =
   {M_IMAGE_PATH MIL_TEXT("CylinderFinder/Cylinders.mbufc"),
    M_IMAGE_PATH MIL_TEXT("CylinderFinder/CylindersWithClutter.ply")};

static const MIL_DOUBLE FIT_DISTANCE = 0.3;
...
void SimpleSceneCylinderFinder   (MIL_ID MilSystem, MIL_ID MilDisplay);
void ComplexSceneCylinderFinder  (MIL_ID MilSystem, MIL_ID MilDisplay);
bool CheckForRequiredMILFile     (MIL_STRING FileName);
void AddComponentNormalsIfMissing(MIL_ID MilContainer);
void ShowCylinderResults         (MIL_ID MilResult, MIL_DOUBLE ComputationTime);
...
int MosMain()
   ...
   SimpleSceneCylinderFinder(MilSystem, MilDisplay);

   ComplexSceneCylinderFinder(MilSystem, MilDisplay);
   }
//*****************************************************************************
// Simple scene cylinder Model finder
//*****************************************************************************
void SimpleSceneCylinderFinder(MIL_ID MilSystem, MIL_ID MilDisplay)
   {
   ...
   // Allocate a cylinder Finder context.
   auto MilContext = M3dmodAlloc(MilSystem, M_FIND_CYLINDER_CONTEXT, M_DEFAULT, M_UNIQUE_ID);
   // Allocate a Cylinder Finder result.
   auto MilResult = M3dmodAllocResult(MilSystem, M_FIND_CYLINDER_RESULT, M_DEFAULT, M_UNIQUE_ID);

   // Define the cylinder model.
   MIL_DOUBLE MinRadius = 2;
   MIL_DOUBLE MaxRadius = 10;
   MIL_DOUBLE MinLength = 3;
   MIL_DOUBLE MaxLength = 50;
   M3dmodDefine(MilContext, M_ADD, M_CYLINDER_RANGE, MinRadius, MinLength, MaxRadius, MaxLength,
                M_WITHOUT_BASES, M_DEFAULT, M_DEFAULT);

   MosPrintf(MIL_TEXT("A cylinder finder is defined with radii range of (%.1f, %.1f) \n"),
             MinRadius, MaxRadius);
   MosPrintf(MIL_TEXT("and lengths range of (%.1f, %.1f).\n\n"), MinLength, MaxLength);
   ...
   // The cylinder finder requires the existence of M_COMPONENT_NORMALS_MIL in the point cloud.
   AddComponentNormalsIfMissing(MilContainer);

   MosPrintf(MIL_TEXT("3D cylinder finder is running..\n"));
   ...
   switch(Status)
      ...
      case M_NOT_ENOUGH_MEMORY:
         MosPrintf(MIL_TEXT("Cylinder finding failed: not enough memory.\n\n"));
         break;
      case M_NOT_ENOUGH_VALID_DATA:
         MosPrintf(MIL_TEXT("Cylinder finding failed: not enough valid points in the point cloud.\n\n"));
         break;
      case M_MISSING_COMPONENT_NORMALS_MIL:
         MosPrintf(MIL_TEXT("Cylinder finding failed: M_COMPONENT_NORMALS_MIL is not found\n")
                   MIL_TEXT("in the point cloud.\n\n"));
         break;
      case M_COMPLETE: {ShowCylinderResults(MilResult, ComputationTime);
                        // Draw all occurrences.
                        M3dmodDraw3d(M_DEFAULT, MilResult, M_ALL,
                                     MilGraphicsList, M_DEFAULT, M_DEFAULT);
                        }break;
      default:  break;
      }

   MosPrintf(MIL_TEXT("Press <Enter> for the next example.\n\n"));
   MosGetch();
   }
//*****************************************************************************
// Complex scene Cylinder Model finder
//*****************************************************************************
void ComplexSceneCylinderFinder(MIL_ID MilSystem, MIL_ID MilDisplay)
   {
   ...
   M3ddispSetView(MilDisplay, M_AZIM_ELEV_ROLL, 4.15, -75, 275, M_DEFAULT);
   M3ddispSelect(MilDisplay, MilContainer, M_SELECT, M_DEFAULT);
   ...
   MIL_DOUBLE Radius = 40;
   MIL_DOUBLE RadiusTolerance = 5;
   MIL_DOUBLE Length = 60;
   MIL_DOUBLE LengthTolerance = 20;
   M3dmodDefine(MilContext, M_ADD, M_CYLINDER, Radius, Length, RadiusTolerance, LengthTolerance,
                M_WITHOUT_BASES, M_DEFAULT, M_DEFAULT);
   MIL_INT ModelIndex = 0;

   MIL_INT MaxNumber = M_ALL;
   MIL_DOUBLE Certainty = 80;
   MIL_DOUBLE MaxCoverage = 40;
   M3dmodControl(MilContext, ModelIndex, M_NUMBER, MaxNumber);
   M3dmodControl(MilContext, ModelIndex, M_CERTAINTY, Certainty);
   M3dmodControl(MilContext, M_DEFAULT, M_FIT_NORMALS_DISTANCE, 5);
   M3dmodControl(MilContext, ModelIndex, M_RESERVED_POINTS_DISTANCE, 10);
   M3dmodControl(MilContext, ModelIndex, M_COVERAGE_MAX, MaxCoverage);

   MosPrintf(MIL_TEXT("Key Controls \n"));
   MosPrintf(MIL_TEXT("----------------------------\n"));
   MosPrintf(MIL_TEXT(" M_NUMBER:           M_ALL\n"));
   MosPrintf(MIL_TEXT(" M_COVERAGE_MAX:     %.2f \n"), MaxCoverage);
   MosPrintf(MIL_TEXT(" M_RADIUS:           %.2f \n"), Radius);
   MosPrintf(MIL_TEXT(" M_LENGTH:           %.2f \n"), Length);
   MosPrintf(MIL_TEXT(" M_TOLERANCE_RADIUS: %.2f \n"), RadiusTolerance);
   MosPrintf(MIL_TEXT(" M_TOLERANCE_LENGTH: %.2f \n\n"), LengthTolerance);
   ...
   ShowCylinderResults(MilResult, ComputationTime);

   // Redraw all occurrences and remove the single drawn ones
  
   auto Label =
      M3dmodDraw3d(MilDrawContext, MilResult, M_ALL, MilGraphicsList, M_DEFAULT, M_DEFAULT);
   MosPrintf(MIL_TEXT("Reserved points will be drawn as well.\n"));
   MosPrintf(MIL_TEXT("Press <ENTER> to continue.\n\n"));
   MosGetch();
   M3dgraRemove(MilGraphicsList, Label, M_DEFAULT);
   M3dmodControlDraw(MilDrawContext, M_DRAW_RESERVED_POINTS, M_ACTIVE, M_ENABLE);
   Label =
      M3dmodDraw3d(MilDrawContext, MilResult, M_ALL, MilGraphicsList, M_DEFAULT, M_DEFAULT);

   MosPrintf(MIL_TEXT("Cylinder finder is run to find complete length cylinders only.\n"));
   ...
   M3dgraRemove(MilGraphicsList, Label, M_DEFAULT);
   M3dmodControlDraw(MilDrawContext, M_DRAW_RESERVED_POINTS, M_ACTIVE, M_DISABLE);
   ...
   MosPrintf(MIL_TEXT("M_LENGTH:           %.2f \n"), Length);
   MosPrintf(MIL_TEXT("M_TOLERANCE_LENGTH: %.2f \n"), LengthTolerance);
   MosPrintf(MIL_TEXT("M_FIT_DISTANCE:     %.2f \n\n"), FIT_DISTANCE);

   M3dmodControl(MilContext, 0, M_TOLERANCE_LENGTH, LengthTolerance);
   M3dmodControl(MilContext, 0, M_LENGTH, Length);
   M3dmodControl(MilContext, M_DEFAULT, M_FIT_DISTANCE_MODE, M_USER_DEFINED); 
   M3dmodControl(MilContext, M_DEFAULT, M_FIT_DISTANCE, FIT_DISTANCE);
...
//*****************************************************************************
// Show the cylinder finder results.
//*****************************************************************************
void ShowCylinderResults(MIL_ID MilResult, MIL_DOUBLE ComputationTime)
   {
   MIL_INT NbOcc = 0;
   ...
   MosPrintf(MIL_TEXT("Index        Error        Score       Radius     Length \n "));
   MosPrintf(MIL_TEXT("------------------------------------------------------\n"));
   ...
   for(MIL_INT i = 0; i < NbOcc; ++i)
      ...
      MIL_DOUBLE Length = M3dmodGetResult(MilResult, i, M_LENGTH, M_NULL);

      MosPrintf(MIL_TEXT("  %i          %.4f        %6.2f       %5.2f      %.2f    \n"),
                i, Error, Score, Radius, Length);
      }
   MosPrintf(MIL_TEXT("\n"));
   }
