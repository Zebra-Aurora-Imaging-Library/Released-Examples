...
// Setup image.
static MIL_CONST_TEXT_PTR IMAGE_FILE = M_IMAGE_PATH MIL_TEXT("FixturedCodeRead/BoardSetup.mim");

// Number of images.
static const MIL_INT NUMBER_OF_IMAGES = 5L;

// More images.
static MIL_CONST_TEXT_PTR ImageFileName[NUMBER_OF_IMAGES] =
   {
...
int MosMain(void)
   ...
   MdispControl(MilDisplay, M_GRAPHIC_LIST_OPACITY, 10.0);

   // Setup display such that it will display the region by
   // painting all the pixels outside the region.
   MdispControl(MilDisplay, M_REGION_OUTSIDE_SHOW, M_GRAPHIC_LIST_OPACITY);
...
void CFixturedCodeRead::Free()
   {
   if(m_MilModelContext      != M_NULL)  MpatFree(m_MilModelContext);
...
void CFixturedCodeRead::Setup(MIL_ID MilSystem, MIL_ID MilDisplay, MIL_ID MilOverlayImage, MIL_ID MilSrcImage)
   ...
   MpatAlloc(MilSystem, M_NORMALIZED, M_DEFAULT, &m_MilModelContext);
   MpatDefine(m_MilModelContext, M_REGULAR_MODEL, MilSrcImage,
              MODEL_OFFSET_X, MODEL_OFFSET_Y,
              MODEL_SIZE_X,   MODEL_SIZE_Y, M_DEFAULT);

   // Set search angle range for model.
   MpatControl(m_MilModelContext, 0, M_SEARCH_ANGLE_MODE, M_ENABLE);
   MpatControl(m_MilModelContext, 0, M_SEARCH_ANGLE_DELTA_NEG, 180.0);
   MpatControl(m_MilModelContext, 0, M_SEARCH_ANGLE_DELTA_POS, 180.0);
   MpatControl(m_MilModelContext, 0, M_SEARCH_ANGLE_ACCURACY, 0.5);

   // Preprocess the model.
   MpatPreprocess(m_MilModelContext, M_DEFAULT, M_NULL);

   // Draw the model.
   MdispControl(MilDisplay, M_OVERLAY_CLEAR, M_DEFAULT);
   MgraColor(M_DEFAULT, M_COLOR_RED);
   MgraControl(M_DEFAULT, M_INPUT_UNITS, M_PIXEL);
   MpatDraw(M_DEFAULT, m_MilModelContext, MilOverlayImage, M_DRAW_BOX+M_DRAW_POSITION, M_DEFAULT, M_ORIGINAL);
   ...
   McalFixture(M_NULL, m_MilFixturingOffset, M_LEARN_OFFSET, M_MODEL_PAT, m_MilModelContext, MilSrcImage, 0, M_DEFAULT, M_DEFAULT);
...
void CFixturedCodeRead::ReadCodeWithRegion(MIL_ID MilSrcImage, MIL_ID MilOverlayImage)
   ...
   // Get decoding status.
   MIL_INT ReadStatus;
   McodeGetResult(m_MilCodeResult, M_GENERAL, M_GENERAL, M_STATUS+M_TYPE_MIL_INT, &ReadStatus);
   ...
   if (ReadStatus == M_STATUS_READ_OK)
      ...
      McodeGetResult(m_MilCodeResult, 0, M_GENERAL, M_STRING + M_STRING_SIZE+M_TYPE_MIL_INT, &ResultStringSize);
      MIL_TEXT_CHAR* ResultString = new MIL_TEXT_CHAR[ResultStringSize]; // Array of characters read.
      McodeGetResult(m_MilCodeResult, 0, M_GENERAL, M_STRING, ResultString);
      ...
      MgraColor(M_DEFAULT, M_COLOR_GREEN);
      McodeDraw(M_DEFAULT, m_MilCodeResult, MilOverlayImage, M_DRAW_BOX, 0, M_GENERAL, M_DEFAULT);

      // Get coordinates of the code.
      double PositionX, PositionY;
      McodeGetResult(m_MilCodeResult, 0, M_GENERAL, M_POSITION_X, &PositionX);
      McodeGetResult(m_MilCodeResult, 0, M_GENERAL, M_POSITION_Y, &PositionY);
