...
void DotSpacing(MIL_ID MilSystem, MIL_ID MilSrcImage, MIL_ID MilDisplay, MIL_INT CodeType)
   ...
   for(MIL_INT ii = 0; ii < 2 && (Status != M_STATUS_READ_OK); ii++)
      ...
      if (Status == M_STATUS_READ_OK)
         ...
         McodeDraw(M_DEFAULT, MilCodeResult, MilOverlayImage, M_DRAW_BOX, M_ALL, M_GENERAL, M_DEFAULT);
         ...
         for(MIL_INT n=0; n < (ResultStringSize-1); n++)
            {
         ...
         MIL_CONST_TEXT_PTR PrefixString = MIL_TEXT("Read string: ");
         ...
         McodeGetResult(MilCodeResult, 0, M_GENERAL, M_BOTTOM_LEFT_X, &BottomLeftX);
         McodeGetResult(MilCodeResult, 0, M_GENERAL, M_BOTTOM_LEFT_Y, &BottomLeftY);
         ...
         if(ii == 0)
            ...
            McodeControl(MilCodeContext, M_DOT_SPACING_MIN, 5);
            McodeControl(MilCodeContext, M_DOT_SPACING_MAX, 7);
            }
         }
      }
