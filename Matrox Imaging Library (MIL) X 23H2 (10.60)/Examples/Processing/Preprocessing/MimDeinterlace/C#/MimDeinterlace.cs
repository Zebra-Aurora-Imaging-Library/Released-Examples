...
namespace MImDeinterlace
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            MIL_ID[] MilDisplayImage = new MIL_ID[1];   /* Image buffer identifier.        */
            MIL_ID[] MilSourceImage  = new MIL_ID[1];   /* Image buffer identifier.        */
            ...
            MIL_INT ImageType = 0;
            MilDisplayImage[0] = MIL.M_NULL;
            MilSourceImage[0] = MIL.M_NULL;
            ...
            MIL.MbufRestore(IMAGE_FILE, MilSystem, ref MilSourceImage[0]);

            /* Allocate a display buffers and show the source image. */
            MIL.MbufInquire(MilSourceImage[0], MIL.M_SIZE_BAND, ref ImageSizeBand);
            MIL.MbufInquire(MilSourceImage[0], MIL.M_SIZE_X, ref ImageWidth);
            MIL.MbufInquire(MilSourceImage[0], MIL.M_SIZE_Y, ref ImageHeight);
            MIL.MbufInquire(MilSourceImage[0], MIL.M_TYPE, ref ImageType);
            MIL.MbufAllocColor(MilSystem, ImageSizeBand, ImageWidth, ImageHeight, ImageType, MIL.M_IMAGE + MIL.M_PROC + MIL.M_DISP, ref MilDisplayImage[0]);
            MIL.MbufCopy(MilSourceImage[0], MilDisplayImage[0]);
            MIL.MdispSelect(MilDisplay, MilDisplayImage[0]);
            ...
            /* Free objects. */
            MIL.MbufFree(MilSourceImage[0]);
            MIL.MbufFree(MilDisplayImage[0]);
        ...
        static void GrabDeinterlace(MIL_ID MilSystem)
        {
            ...
            MIL_ID[] MilCurrentGrabImage = new MIL_ID[1];
            ...
            MIL.MbufAllocColor(MilSystem,
                           ...
                           ref MilDisplayImage[0]);
            MIL.MdispAlloc(MilSystem,
                       MIL.M_DEFAULT,
                       "M_DEFAULT",
                       MIL.M_DEFAULT,
                       ref MilDisplay);
            MIL.MbufClear(MilDisplayImage[0], 0);
            MIL.MdispSelect(MilDisplay, MilDisplayImage[0]);
            ...
            MIL.MdigGrabContinuous(MilDigitizer, MilDisplayImage[0]);
            Console.ReadKey();
            ...
            while (!Console.KeyAvailable)
            {
                ...
                MilCurrentGrabImage[0] = MilGrabImages[CurrentGrabIndex];

                if ((GRAB_DEINTERLACE_METHOD != MIL.M_ADAPTIVE_DISCARD) && (GRAB_DEINTERLACE_METHOD != MIL.M_ADAPTIVE_AVERAGE))
                {
                    /* Deinterlace the current grabbed image. */
                    MIL.MimDeinterlace(MilDeinterlaceContext, MilCurrentGrabImage, MilDisplayImage, 1, 1, MIL.M_DEFAULT);
                }
                else
                {
