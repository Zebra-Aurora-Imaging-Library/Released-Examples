...
//            moving the view in one of the displays also moves all the others.
//
...
class CDisplayLinker
   ...
   public:
      ...
      void SetDisplays(std::vector<MIL_ID> Displays);

      ~CDisplayLinker() { StopLink(); }

   private:
      static const MIL_INT POLLING_DELAY = 50;  // The polling delay, in ms.

      bool                 m_Exit = false;      // Tells the polling thread when to exit.
      std::vector<MIL_ID>  m_Displays;          // List of linked displays.
      MIL_UNIQUE_THR_ID    m_PollingThread;     // Thread which continuously polls and updates the displays.
...
// If several displays are updated at once, the one which comes first in the vector has priority.
//-----------------------------------------------------------------------------
...
// Links the displays.
//-----------------------------------------------------------------------------
void CDisplayLinker::LinkDisplays()
   {
...
// Sets the displays.
//-----------------------------------------------------------------------------
void CDisplayLinker::SetDisplays(std::vector<MIL_ID> Displays)
   {
   StopLink();
   m_Displays = Displays;
   LinkDisplays();
...
void CDisplayLinker::DoPolling() const
   ...
   while(!m_Exit)
      ...
      for(const auto& Display : m_Displays)
         ...
         // Multiply with the inverse of the previous view. If this does not give the identity, the view has changed.
